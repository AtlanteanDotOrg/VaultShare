...
public async Task<Vault> CreateVirtualCardForVaultAsync(Vault vault, long spendingLimit)
{
    ...
    catch (StripeException ex)
    {
        ...
        return GenerateMockCardData(vault.Name);
    }
    catch (Exception ex)
    {
    ...
    // Mock data generator to simulate a virtual card
    private Vault GenerateMockCardData(string vaultName)
    {
        Random rand = new Random();
        Console.WriteLine("Generating mock card data for vault: " + vaultName);

        return new Vault
        {
            CardId = "mock_card_" + Guid.NewGuid().ToString("N").Substring(0, 8),
            CardNumber = $"{rand.Next(1000, 9999)}-{rand.Next(1000, 9999)}-{rand.Next(1000, 9999)}-{rand.Next(1000, 9999)}",
            CardExpiry = $"{rand.Next(1, 12):D2}/{rand.Next(23, 30)}", // Random month/year for expiry
            CardCvc = rand.Next(100, 999).ToString(),
            CardNickname = "Vault Mock Card"
        };
    }
}
