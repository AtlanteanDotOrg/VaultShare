...
public IActionResult Vault(string vaultId)
{
    ...
    var userVaults = _userService.GetUserByGoogleIdAsync(ViewData["GoogleId"].ToString()).Result.Vaults;
    var selectedVault = userVaults.FirstOrDefault(v => v.VaultId == vaultId);

    if (selectedVault == null)
    {
        return RedirectToAction("Dashboard"); // Redirect if the vault is not found
    }
